...
from textooze.augmenters.contextual_replacer import contextual_replacer
...
class TestContextualReplaces:
    ru_texts = ru_texts[:5]
    ru_labels = ru_labels[:5]
    ru_big_text = ' '.join(ru_texts)

    def test_translation(self):
        new_texts, new_labels = contextual_replacer(
            texts=self.ru_texts,
    ...
    def test_new_texts(self):
        new_texts, _ = contextual_replacer(
            texts=self.ru_texts,
            labels=self.ru_labels,
            top_k=3
        )
    ...
    def test_new_labels(self):
        _, new_labels = contextual_replacer(
            texts=self.ru_texts,
            labels=self.ru_labels,
            top_k=3
        )
